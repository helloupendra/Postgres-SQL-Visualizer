...
import { Key, Link as LinkIcon, Table2, Loader2, Search } from "lucide-react";
...
export function SchemaVisualizer() {
  ...
  const [rawSchemaData, setRawSchemaData] = useState<any[]>([]);
  const [selectedSchema, setSelectedSchema] = useState<string>("all");

  const availableSchemas = useMemo(() => {
    const schemas = new Set<string>();
    rawSchemaData.forEach(table => {
  ...
  // Handle Search & Filtering logic locally on the fetched data
  useEffect(() => {
    const filteredTables = selectedSchema === "all" 
      ? rawSchemaData 
      : rawSchemaData.filter(t => t.schema === selectedSchema);

    const tableIds = new Set(filteredTables.map(t => t.id));

    const newNodes = filteredTables.map((table, index) => ({
      id: table.id,
      type: "tableNode",
      position: { x: (index % 3) * 300, y: Math.floor(index / 3) * 250 },
      data: { ...table, searchQuery },
    ...
    setNodes(newNodes);
    setEdges(newEdges);
  }, [searchQuery, selectedSchema, rawSchemaData, setNodes, setEdges]);
